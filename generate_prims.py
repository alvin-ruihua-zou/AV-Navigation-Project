...
def vehicle_update(t, x, u, params):
    ...
    l = params.get("wheelbase", 3.0)  # vehicle wheelbase
...
#             vehicle, [0, 0, -np.pi * 2], [120, 100, np.pi * 2]
#         )
#         if xf[0] + xf[1] <= 0:
#             constraints = [
#                 opt.input_range_constraint(vehicle, [-3, -np.pi / 12], [5, np.pi / 12]),
#                 state_constraint,
#             ]
#         else:
#             constraints = [
#                 opt.input_range_constraint(vehicle, [0, -np.pi / 12], [5, np.pi / 12]),
#                 state_constraint,
#             ]
...
#         terminal = [
#             opt.input_range_constraint(vehicle, uf - 0.01, uf + 0.01),
#             opt.state_range_constraint(
#                 vehicle,
#                 np.array(prims.prims[i].endpose) * prims.resolution - epsilon,
#                 np.array(prims.prims[i].endpose) * prims.resolution + epsilon,
#             ),
#         ]
...
#         )
#         print(
#             f"i: {i}, time: {time}, desired: {xf} state: {result.states[:,-1]} succ: {result.success}"
#         )
#         if not result.success:
#             continue
#         # Simulate the system dynamics (open loop)
#         resp = ct.input_output_response(
#             vehicle, timepts, result.inputs, x0, t_eval=np.linspace(0, Tf, 10)
#         )
